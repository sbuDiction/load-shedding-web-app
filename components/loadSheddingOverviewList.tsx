    ...
    const fetchSchedules = async () => {
        ...
        subscriptions.forEach(async subs => {
            await ScheduleAPI.fetchSchedule(subs.sid).then(res => {
                const currentEevent: Event = LoadSheddingUtils.findUpcomingEvent(res.schedule.days[0].stages);

                const timeLeftBefore = LoadSheddingUtils.timeLeftBeforeEvenStart(currentEevent ? currentEevent.start : '');
                // const timeLeftAfter = LoadSheddingUtils.timeLeftAfterEvenHasStarted([currentEevent]);
                const eventState = LoadSheddingUtils.isEventStarted(currentEevent ? currentEevent.start : '');
                ...
                const overview: ScheduleOverview = {
                    ...
                    timeLeft: !eventState.isStarted ? timeLeftBefore : `(${currentEevent?.end})`,
                    isStarted: eventState.isStarted
                }
                setSuburbs(prevData => [...prevData, overview]);
                setIsLoadShedding(res.schedule.days[0].stages.length > 0 ? true : false)
            });
        });
